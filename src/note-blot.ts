...
import { AuthManager, setOnLoginCallback } from './auth';
...
function generateThoughts () {
   ...
   let tillThereWasYou: string[] = [
    ...
    let withinYouWithoutYou: string[] = [
...
export class EventManager { // BETTER NAME FOR GLOBAL APP HANDLER
    currentThought: Thought | null = null;
    connectingThought: Thought | null = null;
    thoughtsContainer: HTMLElement | null = null;
    currentPage: HTMLElement | null = null;
    endOfPageMarker: HTMLElement | null = null;
    ...
    constructor() {
        ...
        const observer = new IntersectionObserver(([entry]) => {
            ...
            if (this.currentPage.childElementCount > 0 && Array.from(this.currentPage.children).some(child => (child as HTMLElement).textContent?.trim())) {
                this.currentPage = this.createNewPage();
    ...
    setCurrentThought(thought: Thought | null) {
    ...
    setupEventListeners() {
        ...
        document.addEventListener('pointerup', (e) => { // It'd be much better to add an entity and state system for this
            if (e.pointerType == 'mouse') {
                mouseDown = false;
    ...
    createThoughtAtPosition(txt: string, x: number, y: number): Thought {
    ...
    addThoughtToPage(thought: Thought, showText: boolean = false) {
        ...
        if (showText) thought.updateText();
        this.updateEndOfPageMarkerStyle();
    }
    ...
    activateThought(thought: Thought) {
