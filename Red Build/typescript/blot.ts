...
export class Blot {
    ...
    currentMessage: string = ""//"Hello I'm Blot.";
    messageSequence: string[] = [
        "Hello I'm Blot.",
        "Click and drag me anywhere on the screen.",
        "Move your cursor and start typing to create a new thought.",
    ]
    targetThought: Thought | null = null; // Should be Thought type when available
    menuItems: Array<{text: string, action: string, angle: number}>;
    ...
    constructor(em: EventManager) {
        ...
        this.element = document.getElementById('inkBlot') as HTMLElement;
    ...
    getCurrentCenterOffsets(): {centerX: number, centerY: number} {
        const currentWidth = this.menuExpanded ? this.menuWidth : this.blotWidth;
        const currentHeight = this.menuExpanded ? this.menuHeight : this.blotHeight;
        return {
    ...
    drag(deltaX: number, deltaY: number, mouseX: number, mouseY: number): void {
    ...
    updateTarget(): void { // goes to current thought or center if null
    ...
    updateTargetThoughtPosition(): void {
        const { centerX, centerY } = this.getCurrentCenterOffsets();
    ...
    executeAction(action: string): void {
        ...
        switch (action) {
            ...
            case 'lower':
                ...
                if ((window as any).eventManager) (window as any).eventManager.saveThoughts();
                break;
            case 'move':
